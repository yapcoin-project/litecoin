...
Below are some notes on how to build Yapcoin Core for Windows.

The options known to work for building Yapcoin Core on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the Yapcoin Core Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the Yapcoin Core applications.
...
Note that for WSL the Yapcoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/yapcoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
This means you cannot use a directory that located directly on the host Windows file system to perform the build.

The next three steps are an example of how to acquire the source in an appropriate way.

    cd /usr/src
    sudo git clone https://github.com/yapcoin-project/yapcoin.git
    sudo chmod -R a+rw yapcoin
...
way. This will install to `c:\workspace\yapcoin`, for example:

    make install DESTDIR=/mnt/c/workspace/yapcoin
...
causes two of the yapcoin executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the yapcoin code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the yapcoin source code).
