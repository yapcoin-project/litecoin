...
*Setup instructions for a Gitian build of Yapcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Yapcoin
...
- [Building Yapcoin Core](#building-yapcoin-core)
...
Clone the git repositories for yapcoin and Gitian.
...
in the yapcoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Yapcoin Core
----------------

To build Yapcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the yapcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/yapcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/yapcoin-project/yapcoin
...
For example:
```bash
URL=https://github.com/thrasher-/yapcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit yapcoin=${COMMIT} --url yapcoin=${URL} ../yapcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit yapcoin=${COMMIT} --url yapcoin=${URL} ../yapcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit yapcoin=${COMMIT} --url yapcoin=${URL} ../yapcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the yapcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../yapcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/yapcoin
SIGPATH=/some/root/path/yapcoin-detached-sigs

./bin/gbuild --url yapcoin=${BTCPATH},signature=${SIGPATH} ../yapcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/yapcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/yapcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/yapcoin-osx-build.assert
```
...
[yapcoin-project/gitian.sigs.ltc](https://github.com/yapcoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
