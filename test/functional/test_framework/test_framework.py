...
class BitcoinTestFramework(object):
    ...
    def main(self):
        ...
        parser.add_option("--nocleanup", dest="nocleanup", default=False, action="store_true",
                          help="Leave yapcoinds and test.* datadir on exit or error")
        parser.add_option("--noshutdown", dest="noshutdown", default=False, action="store_true",
                          help="Don't stop yapcoinds after the test execution")
        parser.add_option("--srcdir", dest="srcdir", default=os.path.normpath(os.path.dirname(os.path.realpath(__file__)) + "/../../../src"),
                          help="Source directory containing yapcoind/yapcoin-cli (default: %default)")
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: yapcoinds were not stopped and may still be running")
    ...
    def start_node(self, i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None, stderr=None):
        ...
        if binary is None:
            binary = os.getenv("YAPCOIND", "yapcoind")
        args = [binary, "-datadir=" + datadir, "-server", "-keypool=1", "-discover=0", "-rest", "-logtimemicros", "-debug", "-debugexclude=libevent", "-debugexclude=leveldb", "-mocktime=" + str(self.mocktime), "-uacomment=testnode%d" % i]
        if extra_args is not None:
            args.extend(extra_args)
        self.bitcoind_processes[i] = subprocess.Popen(args, stderr=stderr)
        self.log.debug("initialize_chain: yapcoind started, waiting for RPC to come up")
    ...
    def assert_start_raises_init_error(self, i, dirname, extra_args=None, expected_msg=None):
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
            try:
                ...
                assert 'yapcoind exited' in str(e)  # node must have shutdown
                ...
                if expected_msg is None:
                    assert_msg = "yapcoind should have exited with an error"
                else:
                    assert_msg = "yapcoind should have exited with expected error " + expected_msg
    ...
    def _initialize_chain(self, test_dir, num_nodes, cachedir):
        ...
        if create_cache:
            ...
            for i in range(MAX_NODES):
                ...
                args = [os.getenv("YAPCOIND", "yapcoind"), "-server", "-keypool=1", "-datadir=" + datadir, "-discover=0"]
                if i > 0:
    ...
    def _wait_for_bitcoind_start(self, process, datadir, i, rpchost=None):
        ...
        while True:
            if process.poll() is not None:
                raise Exception('yapcoind exited with status %i during initialization' % process.returncode)
            try:
...
class ComparisonTestFramework(BitcoinTestFramework):
    ...
    def add_options(self, parser):
        parser.add_option("--testbinary", dest="testbinary",
                          default=os.getenv("YAPCOIND", "yapcoind"),
                          help="yapcoind binary to test")
        parser.add_option("--refbinary", dest="refbinary",
                          default=os.getenv("YAPCOIND", "yapcoind"),
                          help="yapcoind binary to use for reference nodes (if any)")
